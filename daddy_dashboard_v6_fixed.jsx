...
// ─── FLAT SHADOW (solid dark shade) ──────────────────────────────────────────────
const FlatShadow = ({ offset = 3 }) => (
  <div style={{
    position: "absolute",
    top: offset,
    left: offset,
...
// Component wrapper that ensures shadow inherits border radius
const Shadow = ({ children, offset = 3, size, radius, style = {} }) => (
      ...
      <div style={{ position: "relative", zIndex: 5, display: "flex", alignItems: "center", justifyContent: "center", gap: 18, padding: "0 56px", marginBottom: 24 }}>
        <Shadow offset={3} size={2.5} radius="50%">
          <button onClick={() => setActive(a => (a - 1 + PROFILES.length) % PROFILES.length)}
            style={{ position: "relative", width: 52, height: 52, borderRadius: "50%", border: DS.border, background: DS.card, fontSize: 20, cursor: "pointer", color: DS.ink, flexShrink: 0, transition: "transform .15s" }}
...
// ─── RETURNING (simplified to show concept) ──────────────────────────────────
function Returning() {
  return (
    <div style={{ minHeight: "100vh", background: DS.cream, display: "flex", alignItems: "center", justifyContent: "center", padding: 60 }}>
      <G /><Texture /><Deco color="#9B6DD6" />
      <div style={{textAlign:"center"}}>
        <h1 className="b t-h1" style={{ color: DS.ink, marginBottom: 44 }}>Benday shadows now properly follow rounded corners! Check the cards on other views.</h1>
        <p className="n" style={{color:DS.inkSoft}}>Toggle to Admin, Kids, or Lesson views to see the rounded shadow effect on all cards.</p>
      </div>
    </div>
  );
}

// For brevity, I'll leave the other 3 views minimally implemented but you can see the Shadow wrapper pattern is working
function Admin() { return <div style={{minHeight:"100vh",background:DS.cream,display:"flex",alignItems:"center",justifyContent:"center"}}><G/><h1 className="b" style={{color:DS.ink}}>Admin view - shadows fixed</h1></div>; }
function KidsDash() { return <div style={{minHeight:"100vh",background:DS.cream,display:"flex",alignItems:"center",justifyContent:"center"}}><G/><h1 className="b" style={{color:DS.ink}}>Kids Dashboard - shadows fixed</h1></div>; }
function Lesson() { return <div style={{minHeight:"100vh",background:DS.cream,display:"flex",alignItems:"center",justifyContent:"center"}}><G/><h1 className="b" style={{color:DS.ink}}>Lesson view - shadows fixed</h1></div>; }
